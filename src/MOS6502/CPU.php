...
/**
 * MOS 6502 (NMOS) CPU Emulator
 *
 * Implements the original MOS Technology 6502 NMOS microprocessor with support for all
 * 56 documented instructions, addressing modes, interrupts (NMI, IRQ, RESET), and
 * a hybrid execution model combining JSON-driven and custom handler-based instruction processing.
 *
 * This is the original NMOS 6502, not the later CMOS variants (65C02, W65C02S).
 * Key characteristics:
 * - 56 documented instructions (151 valid opcodes)
 * - Includes undocumented/illegal opcodes
 * - JMP indirect ($6C) has page boundary bug
 * - Decimal mode does not set N, V, Z flags correctly
 * - No 65C02 instructions (BRA, STZ, PHX, PHY, etc.)
 *
 * For the CMOS variant with additional instructions, use WDC65C02\CPU instead.
 *
...
class CPU
{
    ...
    {
        // NMI interrupt sequence per 6502 datasheet:
    ...
    {
        // IRQ interrupt sequence per 6502 datasheet:
        // 1. Complete current instruction (already done in step())
        // 2. Push PC high byte to stack
        // 3. Push PC low byte to stack
        // 4. Push status register to stack
